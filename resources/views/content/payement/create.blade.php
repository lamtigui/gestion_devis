...
@section('content')
    <!-- page title -->
    <x-page-title title="Create Payement For Facture # {{ $facture->n_facture }} " pagetitle="Pages" />

    <div class="tab-content">
        <div class="block tab-pane" id="personalTabs">
            <div class="card">
                <div class="card-body">
                    <h6 class="mb-1 text-15">{{ __('t-createPayement') }}</h6>
                    </p>
                        ...
                        <div class="grid grid-cols-1 gap-5 xl:grid-cols-12">
                            <div class="xl:col-span-6">
                                ...
                                <input type="date" id="inputValue"
                                    ...
                                    placeholder="Enter your value" value="{{ old('date_paiement') }}" name="date_paiement" required>
                                @error('date_paiement')
                            ...
                            <div class="xl:col-span-6">
                                <label for="inputValue" class="inline-block mb-2 text-base font-medium">montant :(max
                                    montant is
                                    {{ $facture->getMantantTTC() - $facture->Payement->sum('montant') }} )</label>
                                <input type="text" id="montantInput" type="number" min="0"
                                    class="form-input border-slate-200 dark:border-zink-500 focus:outline-none focus:border-custom-500 disabled:bg-slate-100 dark:disabled:bg-zink-600 disabled:border-slate-300 dark:disabled:border-zink-500 dark:disabled:text-zink-200 disabled:text-slate-500 dark:text-zink-100 dark:bg-zink-700 dark:focus:border-custom-800 placeholder:text-slate-400 dark:placeholder:text-zink-200"
                                    placeholder="montant de payement" value="{{ old('montant') }}" name="montant" required>
                                @error('montant')
                                    <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                                @enderror
                                <script>
                                    ...
                                    const maxAllowedValue = {{ $facture->getMantantTTC() - $facture->Payement->sum('montant') }};
                        ...
                        <div class="grid grid-cols-1 gap-5 xl:grid-cols-12">
                            <div class="xl:col-span-4">
                                ...
                                <select id="mode_de_payement" name="mode" onchange="toggleFieldsVisibility()" required
                                    ...
                                    <option @selected(old('mode') === 'ESPÈCE') value="ESPÈCE">ESPÈCE</option>
                                    <option @selected(old('mode') === 'VIREMENT ESPÈCE') value="VIREMENT ESPÈCE">VIREMENT ESPÈCE</option>
                                    <option @selected(old('mode') === 'CHEQUE') value="CHEQUE">CHÈQUE</option>
                                    <option @selected(old('mode') === 'VIREMENT') value="VIREMENT">VIREMENT</option>
                                    <option @selected(old('mode') === 'PAR EFFET') value="PAR EFFET">NUMERO D'EFFET</option>
                                </select>
                                @error('mode')
                                    <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                                @enderror
                            ...
                            <div class="xl:col-span-4" id="remiseFields" style="display: none;">
                                ...
                                <input type="text" id="numero_remise"
                                    class="form-input border-slate-200 dark:border-zink-500 focus:outline-none focus:border-custom-500 disabled:bg-slate-100 dark:disabled:bg-zink-600 disabled:border-slate-300 dark:disabled:border-zink-500 dark:disabled:text-zink-200 disabled:text-slate-500 dark:text-zink-100 dark:bg-zink-700 dark:focus:border-custom-800 placeholder:text-slate-400 dark:placeholder:text-zink-200"
                                    placeholder="Numero de remise" value="{{ old('numero_remise') }}" name="numero_remise">
                                @error('numero_remise')
                                    <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div class="xl:col-span-4" id="chequeFields" style="display: none;">
                                ...
                                <input type="text" id="numero_cheque"
                                    class="form-input border-slate-200 dark:border-zink-500 focus:outline-none focus:border-custom-500 disabled:bg-slate-100 dark:disabled:bg-zink-600 disabled:border-slate-300 dark:disabled:border-zink-500 dark:disabled:text-zink-200 disabled:text-slate-500 dark:text-zink-100 dark:bg-zink-700 dark:focus:border-custom-800 placeholder:text-slate-400 dark:placeholder:text-zink-200"
                                    value="{{ old('numero_cheque') }}" name="numero_cheque">
                                @error('numero_cheque')
                                    <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                                @enderror
                            </div>
