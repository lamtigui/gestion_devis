...
@section('content')
    <!-- page title -->
    <x-page-title title="Edit devis" pagetitle="Pages" />
    <div class="tab-content">
        <div class="block tab-pane" id="personalTabs">
            <div class="card">
                <div class="card-body">
                    <h6 class="mb-1 text-15">Edit devis devis #{{ $devi->n_devis }} </h6><br>
                    ...
                    <form action="{{ route('admin.devis.update', $devi) }}" method="POST">
                        @csrf
                        @method('PUT')
                        <div class="grid grid-cols-1 gap-5 xl:grid-cols-12">
                            <div class="xl:col-span-6">
                                ...
                                <input type="text" id="inputValue" required
                                    class="form-input border-slate-200 dark:border-zink-500 focus:outline-none focus:border-custom-500 disabled:bg-slate-100 dark:disabled:bg-zink-600 disabled:border-slate-300 dark:disabled:border-zink-500 dark:disabled:text-zink-200 disabled:text-slate-500 dark:text-zink-100 dark:bg-zink-700 dark:focus:border-custom-800 placeholder:text-slate-400 dark:placeholder:text-zink-200"
                                    value="{{ old('n_devis', $devi->n_devis) }}" name="n_devis">
                                @error('n_devis')
                            ...
                            <div class="xl:col-span-6">
                                ...
                                <input type="date" id="inputValue" required
                                    class="form-input border-slate-200 dark:border-zink-500 focus:outline-none focus:border-custom-500 disabled:bg-slate-100 dark:disabled:bg-zink-600 disabled:border-slate-300 dark:disabled:border-zink-500 dark:disabled:text-zink-200 disabled:text-slate-500 dark:text-zink-100 dark:bg-zink-700 dark:focus:border-custom-800 placeholder:text-slate-400 dark:placeholder:text-zink-200"
                                    value="{{ old('date', $devi->date) }}" name="date">
                                @error('date')
                                    <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                                @enderror
                            </div><!--end col-->
                        ...
                        <div class="grid grid-cols-1 gap-5 xl:grid-cols-12">
                            <div class="xl:col-span-4">
                                ...
                                <select required
                                    ...
                                    <option value="signeé" {{ old('etat', $devi->etat) == 'signeé' ? 'selected' : '' }}>
                                        signeé</option>
                                    <option value="non signeé"
                                        {{ old('etat', $devi->etat) == 'non signeé' ? 'selected' : '' }}>non
                                        signeé
                                    </option>
                                    <option value="en attente de réflexion"
                                        {{ old('etat', $devi->etat) == 'en attente de réflexion' ? 'selected' : '' }}>en
                                        attente de
                                        réflexion</option>
                                </select>
                                @error('etat')
                            ...
                            <div class="xl:col-span-4">
                                ...
                                <select required
                                    ...
                                    <option value="whatssap"
                                        {{ old('mode_envoi', $devi->mode_envoi) == 'whatssap' ? 'selected' : '' }}>
                                        whatssap</option>
                                    <option value="email"
                                        {{ old('mode_envoi', $devi->mode_envoi) == 'email' ? 'selected' : '' }}>
                                        email</option>
                                    <option value="autre"
                                        {{ old('mode_envoi', $devi->mode_envoi) == 'autre' ? 'selected' : '' }}>
                                        Autre
                                    </option>
                                </select>
                                @error('mode_envoi')
                            ...
                            <div class="xl:col-span-4" id="autre_mode_envoi" style="display: none">
                                ...
                                <input type="text" name="autre_mode_denvoi_devis" id="autre_categorie_de_besoin_input"
                                    ...
                                    placeholder="Préciser le mode d'envoi" value="{{ $devi->autre_mode_denvoi_devis }}">
                        ...
                        <div class="xl:col-span-4">
                            ...
                            <input type="number" id="prix_unitaire" name="prix_unitaire" 
                                class="form-input border-slate-200 dark:border-zink-500 focus:outline-none focus:border-custom-500 disabled:bg-slate-100 dark:disabled:bg-zink-600 disabled:border-slate-300 dark:disabled:border-zink-500 dark:disabled:text-zink-200 disabled:text-slate-500 dark:text-zink-100 dark:bg-zink-700 dark:focus:border-custom-800 placeholder:text-slate-400 dark:placeholder:text-zink-200"
                                placeholder="prix unitaire" value={{$devi->prix_unitaire}} required>
                            @error('prix_unitaire')
                                <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                            @enderror
                        </div><!--end col-->
                        <div class="xl:col-span-4">
                            ...
                            <input type="number" id="quantité" name="quantite"
                                class="form-input border-slate-200 dark:border-zink-500 focus:outline-none focus:border-custom-500 disabled:bg-slate-100 dark:disabled:bg-zink-600 disabled:border-slate-300 dark:disabled:border-zink-500 dark:disabled:text-zink-200 disabled:text-slate-500 dark:text-zink-100 dark:bg-zink-700 dark:focus:border-custom-800 placeholder:text-slate-400 dark:placeholder:text-zink-200"
                                placeholder="Quantité" value={{$devi->quantite}} min=1 required>
                            @error('quantité')
                                <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="grid grid-cols-1 gap-5 xl:grid-cols-12">
                            <div class="xl:col-span-4">
                                <label for="productDiscounts" class="inline-block mb-2 text-base font-medium">Prix Hors
                                    Tax</label>
                                    <input type="hidden" id="hors_taxe_value" name="prix_hors_taxe" value="{{ $devi->prix_hors_taxe }}">
                                <input type="number" id="prix_hors_taxe" 
                                    class="form-input border-slate-200 dark:border-zink-500 focus:outline-none focus:border-custom-500 disabled:bg-slate-100 dark:disabled:bg-zink-600 disabled:border-slate-300 dark:disabled:border-zink-500 dark:disabled:text-zink-200 disabled:text-slate-500 dark:text-zink-100 dark:bg-zink-700 dark:focus:border-custom-800 placeholder:text-slate-400 dark:placeholder:text-zink-200"
                                    placeholder="prix hors tax" disabled value="{{ $devi->prix_hors_taxe }}">
                            ...
                            <div class="xl:col-span-4">
                                ...
                                <select id="taux_tva"
                                    ...
                                    <option value="20" @if ($devi->taux_tva == 20) selected @endif>20 %</option>
                                    <option value="0" @if ($devi->taux_tva == 0) selected @endif>0 %</option>
                                    <option value="autre" @if (isset($devi->autre_taux_tva)) selected @endif>autre</option>
                                </select>
                                @error('taux_tva')
                                    <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                                @enderror
                            ...
                            <div class="xl:col-span-6">
                                <label for="productDiscounts" class="inline-block mb-2 text-base font-medium">Remise
                                    01</label>
                                <input type="number" id="remise_01" name="remise_1" value="{{ $devi->remise_1 }}"
                                    class="form-input border-slate-200 dark:border-zink-500 focus:outline-none focus:border-custom-500 disabled:bg-slate-100 dark:disabled:bg-zink-600 disabled:border-slate-300 dark:disabled:border-zink-500 dark:disabled:text-zink-200 disabled:text-slate-500 dark:text-zink-100 dark:bg-zink-700 dark:focus:border-custom-800 placeholder:text-slate-400 dark:placeholder:text-zink-200"
                            ...
                            <div class="xl:col-span-6">
                                <label for="productDiscounts" class="inline-block mb-2 text-base font-medium">date remise
                                    1</label>
                                <input type="date" id="productDiscounts" value="{{ $devi->date_remise_1 }}"
                                    class="form-input border-slate-200 dark:border-zink-500 focus:outline-none focus:border-custom-500 disabled:bg-slate-100 dark:disabled:bg-zink-600 disabled:border-slate-300 dark:disabled:border-zink-500 dark:disabled:text-zink-200 disabled:text-slate-500 dark:text-zink-100 dark:bg-zink-700 dark:focus:border-custom-800 placeholder:text-slate-400 dark:placeholder:text-zink-200"
                            ...
                            <div class="xl:col-span-4">
                                <label for="productDiscounts" class="inline-block mb-2 text-base font-medium">Prix HT
                                    Final</label>
                                <input type="number" id="prixHTFinal" disabled
                                    class="form-input border-slate-200 dark:border-zink-500 focus:outline-none focus:border-custom-500 disabled:bg-slate-100 dark:disabled:bg-zink-600 disabled:border-slate-300 dark:disabled:border-zink-500 dark:disabled:text-zink-200 disabled:text-slate-500 dark:text-zink-100 dark:bg-zink-700 dark:focus:border-custom-800 placeholder:text-slate-400 dark:placeholder:text-zink-200"
                                    placeholder="Prix Hors Tax Final">
                            </div><!--end col-->

                        </div>
                        <script>
                            document.addEventListener('DOMContentLoaded', function() {
                                var prixHorsTaxeHidden = document.getElementById('hors_taxe_value');
                                var prixUnitaireTaxeInput = document.getElementById('prix_unitaire');
                                var quantitéInput = document.getElementById('quantité');
                                var prixHorsTaxeInput = document.getElementById('prix_hors_taxe');
                                ...
                                function calculateAndDisplayValues() {
                                    ...
                                    var tauxTva = tauxTvaSelect.value === 'autre' ? parseFloat(autreTauxTvaInput.value) : parseFloat(
                                        tauxTvaSelect.value);
                                    ...
                                    prixHorsTaxeInput.value = prixHorsTaxe
                                    prixHorsTaxeHidden.value = parseFloat(prixHorsTaxe)
                        ...
                        <div class="flex justify-end mt-6 gap-x-4">
                            <button type="submit"
                                class="text-white btn bg-custom-500 border-custom-500 hover:text-white hover:bg-custom-600 hover:border-custom-600 focus:text-white focus:bg-custom-600 focus:border-custom-600 focus:ring focus:ring-custom-100 active:text-white active:bg-custom-600 active:border-custom-600 active:ring active:ring-custom-100 dark:ring-custom-400/20">Enregistrer les modifications</button>
                        </div>
                    </form><!--end form-->
                </div>
            </div>
